...
def load_all_history_files():
    all_words = []
    # スクリプトがある場所（カレントディレクトリ）を基準にする
    current_dir = os.getcwd()
    
    # history_data*.json をすべて取得
    json_files = glob(os.path.join(current_dir, 'history_data*.json'))
...
def generate_html(data, current_index, sorted_words):
    ...
    return template.format(
        number=data['number'], 
        word=data['word'], 
        era=data.get('era', '時代不明'), 
        description=data.get('description', '解説未記入'), 
        background=data.get('background', '背景情報なし'), 
        related_events=generate_history_link_list(data.get('related', []), sorted_words), 
        location=data.get('location', '不明'), 
        comparisons=generate_history_link_list(data.get('comparisons', []), sorted_words), 
...
def main():
    ...
    # 生成先ディレクトリ：スクリプトと同じ階層にある data フォルダ
    ...
    print("✅ すべての日本史用語HTMLが data/ に生成されました。")
